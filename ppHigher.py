def priority_scheduling(processes, burst_time, arrival_time, priority):
    ...
    sorted_processes = [x for _, x in sorted(zip(priority, processes), reverse=True)]
    sorted_burst_time = [x for _, x in sorted(zip(priority, burst_time), reverse=True)]
    sorted_arrival_time = [x for _, x in sorted(zip(priority, arrival_time), reverse=True)]
...
# Example usage with user input for Priority Scheduling
